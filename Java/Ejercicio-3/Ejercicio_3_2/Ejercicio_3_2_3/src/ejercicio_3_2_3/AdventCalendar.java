...
public class AdventCalendar {
    ...
    private int matrix[][] = new int[6][4];//Matriz de 6 filas y 4 columnas para guardar los 24 dias del calendario
        ...
        for (int i = 0; i < matrix.length && !confirm; i++) {
            for (int j = 0; j < matrix[i].length && !confirm; j++) {
                if (value == matrix[i][j]) {//Comparamos value con todas las posiciones de la matriz
                    confirm = true; //si el numero ya existe confirm es true y se vuelve a generar otro numero random
                }
            }
        }
